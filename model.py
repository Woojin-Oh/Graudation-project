...
def generate_model(opt):
    ...
    if not opt.no_cuda:
        ...
        if opt.pretrain_path:
            ...
            model = modify_kernels(opt, model, opt.pretrain_modality) #여기서는 첫 conv input이 3channel -> 상황에 맞게 바꿔줘야함

            if opt.model_depth == 101:
                model_dict = model.state_dict()
                # 체크포인트에서 fc.weight와 fc.bias 키 제거
                pretrain_dict = {k: v for k, v in pretrain['state_dict'].items() if "fc.weight" not in k and "fc.bias" not in k}
                #pretrain_dict = {k: v for k, v in pretrain['state_dict'].items() if "fc.weight" not in k and "fc.bias" not in k and "module.conv1.weight" not in k}
                #print('pretrain items: ', pretrain['state_dict'].keys())

                # 제거 후 pretrain_dict 업데이트
                model_dict.update(pretrain_dict)
                # state dict 로드
                model.load_state_dict(model_dict)
            else:
                model.load_state_dict(pretrain['state_dict']) #여기서 4channel의 weight 불러옴
            #print('pretrain state_dict keys: ',pretrain['state_dict'].keys())
            #print('pretrain conv1 weight: ', pretrain['state_dict']['module.conv1.weight'].shape) #torch.Size([16, 4, 7, 7, 7])
            ...
            if opt.ft_portion == "svf":
                #model.module.layer1 = resolver(model.module.layer1, global_low_rank_ratio=1.0, skip_1x1=False, skip_3x3=False)
                model.module.layer2 = resolver(model.module.layer2, global_low_rank_ratio=1.0, skip_1x1=False, skip_3x3=False)
                model.module.layer3 = resolver(model.module.layer3, global_low_rank_ratio=1.0, skip_1x1=False, skip_3x3=False)
                model.module.layer4 = resolver(model.module.layer4, global_low_rank_ratio=1.0, skip_1x1=False, skip_3x3=False)
            
                
            
            #model = modify_kernels(opt, model, opt.modality)
            #print('modified model conv1 weight: ', model.state_dict()['module.conv1.weight'].shape)
        else:
            model = modify_kernels(opt, model, opt.modality)

        #parameters = get_fine_tuning_parameters(model, opt.ft_portion)
        get_fine_tuning_parameters(model, opt.ft_portion) #lr = 0 대신 grad_false
...
def _construct_rgbdepth_model(base_model):
    ...
    # modify parameters, assume the first blob contains the convolution kernels
    motion_length = 1
    params = [x.clone() for x in conv_layer.parameters()] #params[0]: weight, params[1]: bias 
    print('params shape: ', params[0].shape) #params shape:  torch.Size([16, 3, 7, 7, 7]) or torch.Size([16, 4, 7, 7, 7])
    kernel_size = params[0].size()
    new_kernel_size = kernel_size[:1] + (1 * motion_length,) + kernel_size[2:]
    print('new kernel size: ', new_kernel_size) #torch.Size([16, 1, 7, 7, 7])
    ...
    new_conv = nn.Conv3d(4, conv_layer.out_channels, conv_layer.kernel_size, conv_layer.stride,
                         conv_layer.padding, bias=True if len(params) == 2 else False)
    print('new_kernels shape: ', new_kernels.shape)
    new_conv.weight.data = new_kernels
    if len(params) == 2:
        new_conv.bias.data = params[1].data  # add bias if neccessary
    layer_name = list(container.state_dict().keys())[0][:-7]  # remove .weight suffix to get the layer name
