...
def video_loader(video_dir_path, frame_indices, modality, sample_duration, image_loader):
    ...
    if modality == 'RGB':
        for i in frame_indices:
            ...
            image_path = os.path.join(video_dir_path,'sk_color_all/', '{:05d}.jpg'.format(i))
        ...
        for i in frame_indices:
            ...
            #image_path = os.path.join(video_dir_path.replace('color','midas'), '{:05d}.jpg'.format(i) )
            image_path = os.path.join(video_dir_path,'sk_midas_all/', '{:05d}.jpg'.format(i))
            if os.path.exists(image_path):
                video.append(image_loader(image_path, modality))
            else:
                print(image_path, "------- Does not exist")
        ...
        for i in frame_indices: # index 35 is used to change img to flow
            ...
            #image_path_depth = os.path.join(video_dir_path.replace('color','midas'), '{:05d}.jpg'.format(i) )
            image_path_depth = os.path.join(image_path.replace('color','midas'))
...
def make_dataset(root_path, annotation_path, subset, n_samples_for_each_video,
    ...
    dataset = []
    print("[INFO]: HOLO Dataset - " + subset + " is loading...")
    for i in range(len(video_names)):
        if i % 1000 == 0:
            print('dataset loading [{}/{}]'.format(i, len(video_names)))

        video_path = os.path.join(root_path,'Video_data' ,video_names[i].replace('./', ''))
